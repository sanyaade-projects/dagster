...
hanging_job = define_asset_job(
    name="hanging_job",
    selection=[first_asset, hanging_asset, never_runs_asset, dummy_source_asset],
)
...
executable_test_job = define_asset_job(
    name="executable_test_job", selection=[executable_asset, unexecutable_asset]
)
...
def _targets_asset_job(instigator: Union[ScheduleDefinition, SensorDefinition]) -> bool:
    ...
    except DagsterInvalidInvocationError:  # thrown when `job_name` is invalid
        return False
